...
- Template: `github-trending-repositories-daily-review` — moved to `csv-templates/github/github-trending-repositories-daily-review/`, display name changed to **GitHub Trending Repositories Daily Review**, and tag list aligned to `github, trending, open-source, discoverability, triage, stars, daily`; workflow inputs and the trending-sync follow-on have been updated to the renamed slug
...
- Template: `github-trending-repositories-daily-review` — primary task renamed from "Evaluate Trending Repos today" to "Evaluate Trending Repos" for cleaner recurrence wording
...
- Workflow: `create-todoist-project.yml` — now automatically triggers after the `Sync GitHub Trending to Todoist` workflow completes successfully, creating the `github-trending-repositories-daily-review` review project as a follow-on step
...
- Gallery: task preview overflow text now says “more entries” (instead of just “more”) to clarify that the count includes both section headers and tasks
- Gallery: **Template Spotlight** task-list preview rows now use brand-consistent red-tint surfaces and accents, while preserving the existing typography and layout
