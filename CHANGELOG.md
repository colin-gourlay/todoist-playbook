...
- Security: **Clickjacking protection** — `app.js` now includes a frame-busting guard that detects when the gallery is loaded inside a frame or iframe and immediately navigates the top frame to the page URL; on cross-origin frames (where navigation is blocked by the same-origin policy) the page content is hidden so it cannot serve as a clickjacking overlay; `assert_gallery_security.py` verifies the guard is present on every build (the CSP `frame-ancestors 'none'` directive is retained in the meta tag for defence-in-depth, but per the CSP Level 2 spec it is ignored there and only effective as an HTTP header)
