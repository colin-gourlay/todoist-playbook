...
User triggers workflow_dispatch, schedule, or workflow_run
  ...
  or let automation choose a default:
    ...
    - Successful trending sync -> github-trending-repositories-daily-review
         │
         ▼
...
- Increments the patch component (`x.y.z → x.y.(z+1)`) and pushes the bump commit to `chore/9000-bump-template-version`
- Opens or updates a pull request from `chore/9000-bump-template-version` into `main` so required checks run before merge
...
- On success, triggers `create-todoist-project.yml` through `workflow_run` so the `github-trending-repositories-daily-review` template can be created automatically
