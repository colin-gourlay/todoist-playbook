...
A review template for discovering, evaluating, and acting on trending GitHub repositories. Use it as a manual weekly review project.
...
- [GitHub Trending Repositories Daily Review](../github/github-trending-repositories-daily-review/)
...
2. `Create Todoist Project from Template` is triggered after a successful trending sync run and creates the **GitHub Trending Repositories Daily Review** project from `template.csv` in `github/github-trending-repositories-daily-review`.
...
3. After the sync succeeds on `main`, the `Create Todoist Project from Template` workflow creates the daily review project automatically using the `github-trending-repositories-daily-review` CSV template
