...
- [GitHub Trending Repositories Daily Review](../github/github-trending-repo-review/)
...
2. `Create Todoist Project from Template` is triggered after a successful trending sync run and creates the **GitHub Trending Repositories Daily Review** project from `template.csv` in `github/github-trending-repo-review`.
...
3. After the sync succeeds on `main`, the `Create Todoist Project from Template` workflow creates the daily review project automatically using the `github-trending-repo-review` CSV template
